...
func _draw_fire() -> void:
	...
	# Heat halo rising off the surface.
	for g in range(3):
		var gh: float = 6.0 * float(g + 1)
		draw_rect(Rect2(0.0, -gh, w, gh), Color(1.0, 0.45, 0.08, 0.10 - 0.025 * float(g)))
	# Back flames: tall, slow, deep red — the wall of fire behind.
	for i in range(maxi(2, int(w / 22.0))):
		var bcx: float = (float(i) + 0.5) * 22.0
		var bfh: float = 15.0 + 11.0 * _flick(_t * 2.6, float(i))
		_draw_tongue(bcx, 9.0, bfh, 4.0 * sin(_t * 1.9 + float(i) * 1.93),
			Color(0.82, 0.20, 0.04, 0.60))
	# Front flames: shorter, fast, bright orange with a yellow core.
	for i in range(maxi(3, int(w / 14.0))):
		var cx: float = (float(i) + 0.86) * 14.0
		var fh: float = 8.0 + 9.0 * _flick(_t * 5.1, float(i) * 1.7)
		var sway: float = 3.2 * sin(_t * 3.1 + float(i) * 2.39)
		_draw_tongue(cx, 6.0, fh, sway, Color(1.0, 0.58, 0.08, 0.88))
		_draw_tongue(cx, 3.1, fh * 0.62, sway * 0.7, Color(1.0, 0.88, 0.35, 0.85))
	# White-hot surface line.
	draw_rect(Rect2(0.0, 0.0, w, 1.6), Color(1.0, 0.92, 0.55, 0.65))
	# Embers sailing up and burning out.
	for j in range(maxi(2, int(w / 34.0))):
		var cycle: float = fposmod(_t * (0.30 + 0.13 * fposmod(float(j) * 0.61, 1.0)) + float(j) * 0.41, 1.0)
		var ex: float = fposmod(float(j) * 34.0 + 13.0, w) + 7.0 * sin(cycle * TAU * 1.4 + float(j))
		var ey: float = -2.0 - cycle * (26.0 + 14.0 * fposmod(float(j) * 0.83, 1.0))
		var fade: float = 1.0 - cycle
		draw_circle(Vector2(ex, ey), 1.1 + 0.8 * fade,
			Color(1.0, 0.55 + 0.35 * fade, 0.15, 0.85 * fade))
